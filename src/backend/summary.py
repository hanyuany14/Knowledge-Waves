    ...
    def llm_summary(self, target_articles: dict[str, list[dict[str, str]]]) -> str:
        ...
        prompt_1 = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    "你是一個友善的學者，負責將文章總結成有意義且重點的段落。請使用繁體中文回覆，並以列點的方式呈現。請將摘要控制在 2-3 句話內。請使用正體中文（繁體中文）回答",
                ),
                ("human", "{content}"),
            ]
        )
        ...
        prompt_2 = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    '你需要將每一篇文章的摘要整合成一段敘述，並使用列點的方式呈現。請將標題（repo 標題或是文章標題）轉換為 <a href="URL">title</a> 的格式插入。最後，請為這些文章撰寫一個總結。請使用繁體中文並保持白話易懂。請使用正體中文（繁體中文）回答',
                ),
                ("human", "{summaries}"),
            ]
        )
